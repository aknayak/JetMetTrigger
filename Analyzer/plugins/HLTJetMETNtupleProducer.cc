...
HLTJetMETNtupleProducer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //iEvent.getByToken(badMuonFilterToken_,badMuonFilter); //old recipe
  ...
  bool pass_badMuonFilter = 1; //*badMuonFilter;
	  ...
	  if( strcmp(triggerName.c_str(),"Flag_HBHENoiseFilter") == 0 )
	  ...
	  else if( strcmp(triggerName.c_str(),"Flag_globalTightHalo2016Filter") == 0 )
	    {
	      if( !isFired ) pass_globalTightHalo2016Filter = 0;
            ...
            {
              if( !isFired ) pass_badMuonFilter = 0;
            }
	}
    }
  ...
  if(!isData_){
    ...
    //Loop over Gen particles and recompute genMET, needed for MonoJet sample
  ...
  muonIsMedium_.clear(); muonIsMediumPrompt_.clear();
  ...
  elec_mva_wp90_Spring16_.clear(); elec_mva_wp80_Spring16_.clear();
  elec_mva_wp90_Fall17Iso_.clear(); elec_mva_wp80_Fall17Iso_.clear(); elec_mva_wpLoose_Fall17Iso_.clear();
  elec_cutId_loose_Summer16_.clear(); elec_cutId_medium_Summer16_.clear(); elec_cutId_tight_Summer16_.clear();
  elec_cutId_loose_Fall17_.clear(); elec_cutId_medium_Fall17_.clear(); elec_cutId_tight_Fall17_.clear();
  ...
  if(Electrons.isValid())
	...
	elec_pass_conversion_.push_back(el->passConversionVeto());
	elecDxy_.push_back(gsfTr_e->dxy(pv_position));
	elecDz_.push_back(gsfTr_e->dz(pv_position));
	elec_mva_wp90_Spring16_.push_back(el->electronID("mvaEleID-Spring16-GeneralPurpose-V1-wp90"));
        elec_mva_wp80_Spring16_.push_back(el->electronID("mvaEleID-Spring16-GeneralPurpose-V1-wp80"));
	elec_mva_wp90_Fall17Iso_.push_back(el->electronID("mvaEleID-Fall17-iso-V2-wp90"));
	elec_mva_wp80_Fall17Iso_.push_back(el->electronID("mvaEleID-Fall17-iso-V2-wp80"));
	//elec_mva_wpLoose_Fall17Iso_.push_back(el->electronID("mvaEleID-Fall17-iso-V2-wpLoose"));
        elec_cutId_loose_Summer16_.push_back(el->electronID("cutBasedElectronID-Summer16-80X-V1-loose"));
        elec_cutId_medium_Summer16_.push_back(el->electronID("cutBasedElectronID-Summer16-80X-V1-medium"));
        elec_cutId_tight_Summer16_.push_back(el->electronID("cutBasedElectronID-Summer16-80X-V1-tight"));
	elec_cutId_loose_Fall17_.push_back(el->electronID("cutBasedElectronID-Fall17-94X-V2-loose"));
        elec_cutId_medium_Fall17_.push_back(el->electronID("cutBasedElectronID-Fall17-94X-V2-medium"));
        elec_cutId_tight_Fall17_.push_back(el->electronID("cutBasedElectronID-Fall17-94X-V2-tight"));
      }
    }
  /*
...
HLTJetMETNtupleProducer::beginJob()
{
  ...
  tree_->Branch("elec_mva_wp90_Spring16", "std::vector<bool>", &elec_mva_wp90_Spring16_);
  tree_->Branch("elec_mva_wp80_Spring16", "std::vector<bool>", &elec_mva_wp80_Spring16_);
  tree_->Branch("elec_mva_wp90_Fall17Iso", "std::vector<bool>", &elec_mva_wp90_Fall17Iso_);
  tree_->Branch("elec_mva_wp80_Fall17Iso", "std::vector<bool>", &elec_mva_wp80_Fall17Iso_);
  //tree_->Branch("elec_mva_wpLoose_Fall17Iso", "std::vector<bool>", &elec_mva_wpLoose_Fall17Iso_);
